...
public class SnowTicketExcelReader {
	private static final Logger LOGGER = LogManager.getLogger();

	/** Creates a new instance of POIExcelReader */
	public SnowTicketExcelReader() {
	}
	/**
	 ...
	 * @param inputStream
	 */
	public List<SnowTicketReport> readExcelFile(File file)
	{
		/**
		 * --Define a List --Holds List Of Cells
		 */
		List<SnowTicketReport> ticketReportList = new ArrayList<SnowTicketReport>();
		try {
			...
			HSSFCell myCell;
			SnowTicketReport ticketReport = null;
			...
			while (rowIter.hasNext()) {
				...
				if (rowNum != 0) {
					...
					ticketReport = new SnowTicketReport();
					ticketMap = new HashMap<String, String>();
					while(cellIter.hasNext()) {
						myCell = (HSSFCell) cellIter.next();
						try {
							ticketMap.put(Constants.snowIncidentMap.get(myCell.getColumnIndex()), getCellValue(myCell).trim().replaceAll("'", "''"));
						} catch(Exception e)
						{ 
							e.printStackTrace();
					...
					BeanWrapper wrapper = new BeanWrapperImpl(SnowTicketReport.class);
					wrapper.setPropertyValues(ticketMap);
					ticketReport = (SnowTicketReport) wrapper.getWrappedInstance();
		...
		System.out.println("ticketReportList Size"+ticketReportList.size());
