...
public class CRExcelReader {
	private static final Logger LOGGER = LogManager.getLogger();

	/** Creates a new instance of POIExcelReader */
	public CRExcelReader() {
	}
	/**
	 ...
	 * @param inputStream
	 */
	public List<ChangeRequestReport> readExcelFile(File file)
	{
		/**
		 * --Define a List --Holds List Of Cells
		 */
		List<ChangeRequestReport> changeRequestReportList = new ArrayList<ChangeRequestReport>();
		try {
			...
			HSSFCell myCell;
			ChangeRequestReport changeRequestReport = null;
			Map<String, String> changeRequestMap = null;
			while (rowIter.hasNext()) {
				...
				if (rowNum != 0) {
					...
					changeRequestReport = new ChangeRequestReport();
					changeRequestMap = new HashMap<String, String>();
					while(cellIter.hasNext()) {
						myCell = (HSSFCell) cellIter.next();
						try {
							changeRequestMap.put(Constants.changeRequestReportMap.get(myCell.getColumnIndex()), getCellValue(myCell).trim().replaceAll("'", "''"));
						} catch(Exception e)
						{ 
							e.printStackTrace();
					...
					BeanWrapper wrapper = new BeanWrapperImpl(ChangeRequestReport.class);
					wrapper.setPropertyValues(changeRequestMap);
					changeRequestReport = (ChangeRequestReport) wrapper.getWrappedInstance();
					changeRequestReportList.add(changeRequestReport);
				} 
		...
		System.out.println("ChangeRequestReportList Size"+changeRequestReportList.size());

		return changeRequestReportList;
	}
